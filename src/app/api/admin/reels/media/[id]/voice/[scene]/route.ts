...
export async function GET(
  ...
  return proxyBinaryMedia(
    request,
    `/jobs/${encodeURIComponent(id)}/media/voice/${encodeURIComponent(scene)}`,
  );
}

async function proxyBinaryMedia(
  request: Request,
  enginePath: string,
  ...
  try {
    const res = await engineFetch(enginePath, { headers });
    if (!res.ok) {
